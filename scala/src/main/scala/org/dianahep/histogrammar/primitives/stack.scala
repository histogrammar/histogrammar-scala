...
package histogrammar {
  ...
  class Stacking[DATUM, V <: Aggregator[DATUM, V]](val expression: NumericalFcn[DATUM], val cuts: (Double, V)*) extends Aggregator[DATUM, Stacking[DATUM, V]] {
    ...
    def +(that: Stacking[DATUM, V]) =
      if (this.cuts.size != that.cuts.size)
        throw new AggregatorException(s"cannot add Stacking because the number of cut differs (${this.cuts.size} vs ${that.cuts.size})")
      else
        new Stacking(this.expression,
          this.cuts zip that.cuts map {case ((mycut, me), (yourcut, you)) =>
            if (mycut != yourcut)
              throw new AggregatorException(s"cannot add Stacking because cut differs ($mycut vs $yourcut)")
            (mycut, me + you)
          }: _*)
