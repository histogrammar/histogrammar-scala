...
package histogrammar {
  ...
  object UntypedLabel extends Factory {
    ...
    def fixed(pairs: (String, Container[_])*) = new UntypedLabeled(pairs: _*)
    def apply[DATUM](pairs: (String, Container[_] with AggregationOnData {type Datum = DATUM})*) = new UntypedLabeling(pairs: _*)

    def unapplySeq(x: UntypedLabeled) = Some(x.pairs)
    def unapplySeq[DATUM](x: UntypedLabeling[DATUM]) = Some(x.pairs)

    def fromJsonFragment(json: Json): Container[_] = json match {
      case JsonObject(pairs @ _*) =>
        new UntypedLabeled(pairs map {
  ...
  class UntypedLabeled(val pairs: (String, Container[_])*) extends Container[UntypedLabeled] {
    ...
    def +(that: UntypedLabeled) =
      if (this.keySet != that.keySet)
        ...
        new UntypedLabeled(this.pairs.map({case (key, mysub) =>
    ...
    override def equals(that: Any) = that match {
      case that: UntypedLabeled => this.pairsMap == that.pairsMap
      case _ => false
    }
    override def hashCode() = pairsMap.hashCode
  }
  ...
  class UntypedLabeling[DATUM](val pairs: (String, Container[_] with AggregationOnData {type Datum = DATUM})*) extends Container[UntypedLabeling[DATUM]] with AggregationOnData {
    ...
    def +(that: UntypedLabeling[DATUM]) =
      if (this.keySet != that.keySet)
        ...
        new UntypedLabeling[DATUM](this.pairs.map({case (key, mysub) =>
          val yoursub = that.pairsMap(key)
          if (mysub.factory != yoursub.factory)
    ...
    override def equals(that: Any) = that match {
      case that: UntypedLabeling[DATUM] => this.pairsMap == that.pairsMap
      case _ => false
    }
    override def hashCode() = pairsMap.hashCode
  }
