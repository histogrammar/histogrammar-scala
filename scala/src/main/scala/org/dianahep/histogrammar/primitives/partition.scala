...
Partition is also similar to [[org.dianahep.histogrammar.CentrallyBin]], in that they both partition a space into irregular subdomains with no gaps and no overlaps. However, CentrallyBin is defined by bin centers and Partition is defined by bin edges, the first and last of which are at negative and positive infinity."""
    ...
    /** Create an immutable [[org.dianahep.histogrammar.Partitioned]] from arguments (instead of JSON).
      ...
      */
    def ed[V <: Container[V] with NoAggregation, N <: Container[N] with NoAggregation](entries: Double, cuts: Iterable[(Double, V)], nanflow: N) = new Partitioned(entries, None, cuts.toSeq, nanflow)
    ...
    def apply[DATUM, V <: Container[V] with Aggregation{type Datum >: DATUM}, N <: Container[N] with Aggregation{type Datum >: DATUM}](cuts: Iterable[Double], quantity: UserFcn[DATUM, Double], value: => V, nanflow: N = Count()) =
      new Partitioning((java.lang.Double.NEGATIVE_INFINITY +: SortedSet(cuts.toSeq: _*).toSeq).map((_, value.zero)), quantity, nanflow, 0.0)
      ...
      case JsonObject(pairs @ _*) if (pairs.keySet has Set("entries", "type", "data", "nanflow:type", "nanflow").maybe("name").maybe("data:name")) =>
        ...
        get("data") match {
          case JsonArray(elements @ _*) if (elements.size >= 1) =>
            new Partitioned[Container[_], Container[_]](entries, (nameFromParent ++ quantityName).lastOption, elements.zipWithIndex map {case (element, i) =>
              element match {
  ...
  /** An accumulated suite of containers, each collecting data between a pair of given cuts on a given quantity.
    ...
    * @param nanflow Container for data that resulted in `NaN`.
    */
  class Partitioned[V <: Container[V] with NoAggregation, N <: Container[N] with NoAggregation] private[histogrammar](val entries: Double, val quantityName: Option[String], val cuts: Seq[(Double, V)], val nanflow: N) extends Container[Partitioned[V, N]] with NoAggregation with QuantityName {
    type Type = Partitioned[V, N]
    type EdType = Partitioned[V, N]
    ...
    def zero = new Partitioned[V, N](0.0, quantityName, cuts map {case (c, v) => (c, v.zero)}, nanflow.zero)
    def +(that: Partitioned[V, N]) = {
  ...
  /** Accumulating a suite of containers, each collecting data between a pair of given cuts on a given quantity.
    ...
    * @param entries Weighted number of entries (sum of all observed weights).
    */
  class Partitioning[DATUM, V <: Container[V] with Aggregation{type Datum >: DATUM}, N <: Container[N] with Aggregation{type Datum >: DATUM}] private[histogrammar](val cuts: Seq[(Double, V)], val quantity: UserFcn[DATUM, Double], val nanflow: N, var entries: Double) extends Container[Partitioning[DATUM, V, N]] with AggregationOnData with NumericalQuantity[DATUM] {
    protected val v = cuts.head._2
    type Type = Partitioning[DATUM, V, N]
    type EdType = Partitioned[v.EdType, nanflow.EdType]
    type Datum = DATUM
    def factory = Partition
    ...
    def zero = new Partitioning[DATUM, V, N](cuts map {case (c, v) => (c, v.zero)}, quantity, nanflow.zero, 0.0)
    def +(that: Partitioning[DATUM, V, N]) = {
      if (this.thresholds != that.thresholds)
        throw new ContainerException(s"cannot add ${getClass.getName} because cut thresholds differ")
