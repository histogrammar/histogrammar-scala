  ...
  implicit def sparselyBinnedToHistogramMethods(hist: Cutted[SparselyBinned[Counted, Counted]]): HistogramMethods =
    if (hist.value.numFilled > 0)
      new HistogramMethods(
        Cut.ed(hist.entries, Bin.ed(hist.value.low.get, hist.value.high.get, 0.0, hist.value.minBin.get to hist.value.maxBin.get map {i => Count.ed(hist.value.at(i).flatMap(x => Some(x.entries)).getOrElse(0L))}, Count.ed(0L), Count.ed(0L), hist.value.nanflow))
      )
    else
